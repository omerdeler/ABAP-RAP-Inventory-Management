...
*    " 1. Kullanıcının ön yüzden seçtiği/girdiği verileri okuyoruz
*    READ ENTITIES OF Z_I_ProductStock_OD IN LOCAL MODE
*      ENTITY ProductStock
...
*
*        " 2. Veritabanında bu ürüne ait eski bir kayıt var mı diye kontrol ediyoruz
*        SELECT SINGLE FROM zinv_stock_od
*          FIELDS stock_id, total_quantity
*          WHERE product_id = @<fs_entity>-product_id
*          INTO @DATA(ls_existing_stock).
*
*        IF sy-subrc = 0.
*          " --- B MANTIĞI TETİKLENİYOR ---
*          " Ürün zaten veritabanında varsa, yeni bir mükerrer satır açılmasını ENGELLİYORUZ:
