    ...
    on $projection.product_id = _Product.product_id
{
    ...
    key stock_id,
      ...
      case
        when total_quantity is null or total_quantity = 0 then 'STOK YOK'
        when total_quantity <= _Product.min_stock_level then 'KRITIK'
        else 'NORMAL'
      end                   as StockStatus,
      
      
      // Fiori için Renk Kodu Alanı
      case
        when total_quantity is null or total_quantity = 0 then 1 // STOK YOK ->Kırmızı (Tehlike) 
        when total_quantity <= _Product.min_stock_level then 2 // KRITIK -> Sarı (Uyarı)
        else 3                                                 // NORMAL -> Yeşil (Güvenli)
